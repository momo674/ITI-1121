...
public class Simulator {
  ...
  public void simulate() {
    ...
    Car cars;
	this.clock = 0;
    boolean ifCarArriving = false;
    Car arrivingCar;
	while (clock < steps) {
      ifCarArriving = RandomGenerator.eventOccurred(this.probabilityOfArrivalPerSec);
      if (ifCarArriving==true){
        cars = RandomGenerator.generateRandomCar();
        spot = new Spot(cars, this.clock);
        incomingQueue.enqueue(spot);
      }
      for (int i = 0; i < this.lot.getNumRows(); i++){
        for (int j = 0; j < this.lot.getNumSpotsPerRow(); j++){
            if (this.lot.getSpotAt(i,j) != null){
                int time_of_selected_spot = (this.lot.getSpotAt(i,j)).getTimestamp();
          if (clock - time_of_selected_spot >= 8 * 3600){
            Spot selected_spot = this.lot.remove(i,j);
            this.outgoingQueue.enqueue(selected_spot);
          }
          else{
                TriangularDistribution leaveCheck = new TriangularDistribution(time_of_selected_spot,(time_of_selected_spot+4*3600),(time_of_selected_spot+8*3600));
                boolean carLeaving = RandomGenerator.eventOccurred(leaveCheck.pdf(clock));
                if (carLeaving){   
                    Spot spotLeaving = this.lot.remove(i,j);
                    outgoingQueue.enqueue(spotLeaving);
    ...
    if (!this.incomingQueue.isEmpty()) {
        Spot attemptingCarSpot = this.incomingQueue.dequeue();
        Car attemptingCar = attemptingCarSpot.getCar();
        if (this.lot.attemptParking(attemptingCar, clock)){
            
            System.out.println(attemptingCar + " ENTERED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy()); 
        }
        else{
            Queue <Spot> tempQueue = new LinkedQueue<Spot>();
            tempQueue.enqueue(attemptingCarSpot);
            while (!this.incomingQueue.isEmpty()){
                tempQueue.enqueue(this.incomingQueue.dequeue());
            }
            this.incomingQueue = tempQueue;
        }
          
      }
      if (!this.outgoingQueue.isEmpty()) {
          Spot removed_spot = this.outgoingQueue.dequeue();
          Car removed_spot_car = removed_spot.getCar();
          System.out.println(removed_spot_car + " EXITED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
      }
      clock++;  
    }
  }
