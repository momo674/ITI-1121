...
public class Simulator {
	...
	public Simulator(ParkingLot lot, int perHourArrivalRate, int steps) {
        if (perHourArrivalRate<1){throw new IllegalArgumentException("Per Hour Arrival Rate must be greater than 1");}
        if (lot == null){throw new IllegalArgumentException("Parking lot cannot be null");}
        if (steps <= 0){throw new IllegalArgumentException("Must be a valid amount of simulated seconds");}
	...
	public void simulate() {
		...
		while (clock < steps) {
			...
			if (incomingToProcess != null) {
                ...
                if (this.lot.attemptParking(incomingToProcess.getCar(), clock)) {
                    incomingQueue.dequeue();
                    this.lot.park(incomingToProcess.getCar(), clock);
                }
			}
