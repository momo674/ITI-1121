...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    public int count(E fromElement, E toElement) {
        ...
        if (toElement == null) {return this.size();}
        if (fromElement.equals(toElement)) {return 1;}
        Node<E> cursor = this.head;
        int count = 0;
        return count(cursor,fromElement,toElement,count);
    }

    private int count(Node<E> p, E fromElement, E toElement, int count) {
        if (p.value.equals(fromElement) && count == 0) { //start counting
            count = 1;
            p = p.next;
            
        }

        else if (p.value.equals(toElement) && count > 0) { //end counting
            count++;
            return count;
        }
